...
- **macOS (Apple Silicon/M1/M2/M3)**: [Download STITCH-macOS-ARM.zip](https://github.com/njw0709/stitch/releases/latest/download/STITCH-macOS-ARM.zip)
  ...
  - **First-time users**: Right-click the app and select "Open" to bypass macOS Gatekeeper (app is unsigned). If it does not show you an option to "Open Anyway" the STITCH.app.
...
1. Selecting survey/interview data with dropdown selection for ID column, date column, and GEOID column
...
python stitch_cli.py \
    --survey-data "path/to/survey_data.dta" \
    --context-dir "path/to/daily_heat_long" \
    --output_name "path/to/linked_data.dta" \
    --id-col hhidpn \
    --date-col iwdate \
    --geoid-col GEOID2010 \
...
- `--survey-data`: Path to survey data file (.dta, csv, parquet, feather, excel)
...
```bash
python stitch_cli.py \
    --survey-data "data/survey_2016.dta" \
...
- Format: Stata (.dta), csv, parquet, feather, excel
- Required columns:
...
- Format: Stata (.dta), csv, parquet, feather, excel file
- Required columns:
  - Participant ID
  - Move indicator
  - Move year/month
